...
const MovieDetails = () => {
  ...
  if (loading) {
    return <div className="container text-center my-5">Loading movie details...</div>;
  }
  ...
  if (!movieData) {
    return <div className="container text-center my-5">No movie data available</div>;
  }
    ...
    <>
      ...
      <div className="movie-container container">
        {movieData.backdrop_path && (
          <div className="movie-poster" style={{ backgroundImage: `url(${BACKDROP_PATH}${movieData.backdrop_path})`,borderRadius:'5px' }}></div>
        )}

        <div className="container-fluid px-4">
          <div className="row movie-header">
            <div className="col-lg-7">
              <h1 className="movie-title">{isMovie ? movieData.title : movieData.name}</h1>

              <div className="movie-rating">
                <span className="rating-star"><i className="fas fa-star"></i></span>
                <span className="fw-bold">{movieData.vote_average.toFixed(1)}</span>
              </div>
              ...
              <div className="movie-tags">
                {movieData.genres && movieData.genres.map((genre) => (
                  <span key={genre.id} className="movie-tag">{genre.name}</span>
                ))}
              </div>
              ...
              <div className="action-buttons">
                ...
                <button className="btn-secondary" onClick={handleWatchLater}>
                  <span className="btn-icon"><i className="fas fa-clock"></i></span>
                  Watch Later
                </button>
              </div>
            </div>
      ...
      <div className="tabs-section container">
        <div className="container-fluid px-4">
          ...
          {/* Recommended Tab */}
          <div id="recommended" className={`tab-content ${activeTab === "recommended" ? "active" : ""}`}>
            <div className="row movie-cards">
              {movieData.recommendations && movieData.recommendations.results.slice(0, 1).map((movie, index) => (
                <div className="col-md-3 col-sm-6 col-lg-2 movie-container" key={movie.id}>
                    {/* <img src="https://image.tmdb.org/t/p/w440_and_h660_face/2ZbQjEQjvFdHXOJsxmUEAV3xVLI.jpg" alt="" srcset="" /> */}
                    <Test movie={movie} index={index} hoverWindow={false} />
                  ...
                  return (
                    <div className="review-card" key={review.id || index}>
                      <div className="review-header">
                        ...
                        {rating && (
                          <div className="review-rating">
                            <i className="fa-solid fa-star"></i> {rating.toFixed(1)}/10
                          </div>
                        )}
                      </div>
                      ...
                      <div className="review-footer">
                        <a href={review.url} target="_blank" rel="noopener noreferrer" className="btn-view-original">
                          <i className="fa-solid fa-external-link"></i> View Original
                        </a>
                      </div>
                    </div>
                  );
  ...
  return (
    <>
      {expanded ? (
        <>
          <p>{content}</p>
          <button className="btn-read-less" onClick={() => setExpanded(false)}>Read less</button>
        </>
      ) : (
        <>
          <p>{shortContent}</p>
          <button className="btn-read-more" onClick={() => setExpanded(true)}>Read more</button>
        </>
      )}
    </>
  );
