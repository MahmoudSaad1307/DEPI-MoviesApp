...
const UserProfile = () => {
  ...
  const COLORS = ['#FF5252', '#FF7B52', '#FFB452', '#FFD452', '#E2FF52', 
                 '#B5FF52', '#7DFF52', '#52FF7B', '#52FFB4', '#52FFE0'];
    ...
    if (active && payload && payload.length) {
      return (
        <div className="custom-tooltip bg-dark text-white p-2 rounded">
          <p className="mb-0">{`${payload[0].name}: ${payload[0].value} movie${payload[0].value !== 1 ? 's' : ''}`}</p>
        </div>
      );
    }
    return null;
        ...
        <div className="container profile-header-container">
            ...
            <div className="col-md-4">
              <div className="d-flex justify-content-end profile-stats">
                <div className="text-center me-4">
                  <div className="stats-number">{watched.watched?.length || 0}</div>
          ...
          <nav className="profile-nav container">
            <ul className="nav nav-tabs">
              ...
              <li className="nav-item">
                <Link className="nav-link" to="/movies4User/watched">Films</Link>
              </li><li className="nav-item">
                <Link className="nav-link" to="/movies4User/watchlist">Watch Later</Link>
              </li><li className="nav-item">
                <Link className="nav-link" to="/movies4User/FAVORITE">Favorites</Link>
              </li>
              <li className="nav-item">
                <Link to="/myReviews" className="nav-link" href="#">Reviews</Link>
              </li>
              <li className="nav-item">
            ...
            <div className="col-md-4">
              <section className="mb-4">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <span className="stats-badge">{user?.ratingsCount || watched.watched?.length || 0}</span>
                </div>
                <div className="ratings-chart">
                  {ratingStats.length > 0 ? (
                      ...
                      <PieChart className='py-3'>
                        ...
                        >
                          {ratingStats.map((entry, index) => (
                            <Cell key={`cell-${index}`} fill={COLORS[entry.rating - 1 % COLORS.length]} />
                          ))}
                        </Pie>
                    ...
                    <div className="text-center py-4">
                      ...
                      <p className="text-muted small">Rate movies to see your distribution</p>
                    </div>
                  )}
                </div>
              </section>
