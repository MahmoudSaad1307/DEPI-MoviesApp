...
import Pagination, { paginate, getPageNumbers, getItemRange } from "../../src/constants/Pagintaion/Pagintaion";
import {
  API_KEY,
  BASE_URL,
  BACKDROP_PATH,
...
const ActorPage = () => {
  ...
  const itemsPerPage = 10; // Matches Pagination default
    ...
    const fetchActorData = async () => {
      try {
        ...
        const response = await fetch(
          `${BASE_URL}${ENDPOINTS.person.details(id)}?api_key=${API_KEY}&append_to_response=movie_credits,tv_credits`
        );
  ...
  const handlePaginateMovies = (pageNumber) => {
    ...
    paginate(pageNumber); // Use the exported paginate function
    window.scrollTo(0, 0);
  };
  ...
  // Use exported functions for pagination
  const { pageNumbers: moviePageNumbers, totalPages: totalMoviePages, startPage: movieStartPage, endPage: movieEndPage } = getPageNumbers(currentMoviePage, movies.length, itemsPerPage);
  const { pageNumbers: tvPageNumbers, totalPages: totalTvPages, startPage: tvStartPage, endPage: tvEndPage } = getPageNumbers(currentTvPage, tvShows.length, itemsPerPage);
  
  const { indexOfFirstItem: movieFirstItem, indexOfLastItem: movieLastItem } = getItemRange(currentMoviePage, movies.length, itemsPerPage);
  const { indexOfFirstItem: tvFirstItem, indexOfLastItem: tvLastItem } = getItemRange(currentTvPage, tvShows.length, itemsPerPage);

  // Slice movies and TV shows based on pagination
        ...
        <div className="col-lg-8">
          <div className="mb-4">
            ...
            <ul className="nav nav-tabs mb-4">
              ...
              <li className="nav-item">
                <button 
                  className={`nav-link ${activeTab === "tvShows" ? "active" : ""}`}
                ...
                <div className="row row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-5 g-3">
                  {movies.length > 0 ? (
                    currentMovies.map((movie,index) => (
                      movie.poster_path &&
                      <MovieCard movie={movie} index={index} isMovie={'movie'}    />
                    ...
                    <div className="pagination-container">
                      <nav aria-label="Page navigation">
                        <ul className="pagination justify-content-center">
                          <li className={`page-item ${currentMoviePage === 1 ? 'disabled' : ''}`}>
                            <button 
                              className="page-link" 
                              onClick={() => handlePaginateMovies(currentMoviePage - 1)}
                          ...
                          {moviePageNumbers.map(number => (
                            <li key={number} className={`page-item ${currentMoviePage === number ? 'active' : ''}`}>
                              <button 
                                className="page-link" 
                                onClick={() => handlePaginateMovies(number)}
                                aria-label={`Page ${number}`}
                                aria-current={currentMoviePage === number ? "page" : null}
                              >
                                {number}
                              </button>
                            </li>
                          ...
                          {movieEndPage < totalMoviePages && (
                            <>
                              ...
                              <li className="page-item">
                                <button 
                                  className="page-link" 
                                  onClick={() => handlePaginateMovies(totalMoviePages)}
                          ...
                          <li className={`page-item ${currentMoviePage === totalMoviePages ? 'disabled' : ''}`}>
                            <button 
                              className="page-link" 
                              onClick={() => handlePaginateMovies(currentMoviePage + 1)}
            ...
            {activeTab === "tvShows" && (
              <>
                <div className="row row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-5 g-3">
                  {tvShows.length > 0 ? (
                    currentTvShows.map((show,index) => (
                      show.poster_path &&
                      <MovieCard movie={show} index={index} isMovie={'tv'}    />
                ...
                {tvShows.length > itemsPerPage && (
                  <div className="mt-4">
                    <div className="pagination-container">
                      <nav aria-label="Page navigation">
                        <ul className="pagination justify-content-center">
                          <li className={`page-item ${currentTvPage === 1 ? 'disabled' : ''}`}>
                            <button 
                              className="page-link" 
                              onClick={() => handlePaginateTvShows(currentTvPage - 1)}
                          ...
                          {tvPageNumbers.map(number => (
                            <li key={number} className={`page-item ${currentTvPage === number ? 'active' : ''}`}>
                              <button 
                                className="page-link" 
                                onClick={() => handlePaginateTvShows(number)}
                                aria-label={`Page ${number}`}
                                aria-current={currentTvPage === number ? "page" : null}
                              >
                                {number}
                              </button>
                            </li>
                          ...
                          {tvEndPage < totalTvPages && (
                            <>
                              ...
                              <li className="page-item">
                                <button 
                                  className="page-link" 
                                  onClick={() => handlePaginateTvShows(totalTvPages)}
                          ...
                          <li className={`page-item ${currentTvPage === totalTvPages ? 'disabled' : ''}`}>
                            <button 
                              className="page-link" 
                              onClick={() => handlePaginateTvShows(currentTvPage + 1)}
